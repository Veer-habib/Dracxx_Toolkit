...
Supports remote OpenAI/Anthropic/Google-compatible APIs and local
...
class OpenAICompatibleProvider(AIProvider):
    """Covers OpenAI, Google (OpenAI-compat endpoint), and local servers
    (Ollama/LM Studio/vLLM) that implement /v1/chat/completions."""
...
def build_provider(cfg) -> AIProvider:
    """cfg is a DracxxConfig.ai + resolved api key."""
    if cfg.mode == "local":
        return OpenAICompatibleProvider(
            model=cfg.local_model, api_key="", temperature=cfg.temperature,
            endpoint=cfg.local_endpoint,
        )
    if cfg.provider == "anthropic":
        import os
        return AnthropicProvider(
            model=cfg.model, api_key=os.environ.get("ANTHROPIC_API_KEY", ""),
            temperature=cfg.temperature, endpoint=cfg.endpoint,
        )
    import os
    key_env = "OPENAI_API_KEY" if cfg.provider == "openai" else "GOOGLE_API_KEY"
    return OpenAICompatibleProvider(
        model=cfg.model, api_key=os.environ.get(key_env, ""),
        temperature=cfg.temperature, endpoint=cfg.endpoint,
    )
